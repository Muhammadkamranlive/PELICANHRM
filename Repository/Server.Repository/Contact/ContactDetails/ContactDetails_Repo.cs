...
namespace Server.Repository
{
    public class ContactDetails_Repo:Repo<CONTACTDETAILS>, IContactDetails_Repo
    {
        ...
        public ContactDetails_Repo(ERPDb db, IHttpContextAccessor httpContextAccessor) :base(db, httpContextAccessor) 
        {
                _db = db;
               _httpContextAccessor = httpContextAccessor;
        }

        public override async Task<IEnumerable<CONTACTDETAILS>> GetAll()
        {
        ...
        public override async Task<IEnumerable<CONTACTDETAILS>> Find(Expression<Func<CONTACTDETAILS, bool>> predicate)
        {
            var tenantId = Convert.ToInt32(_httpContextAccessor.HttpContext?.Items["CurrentTenant"]);
